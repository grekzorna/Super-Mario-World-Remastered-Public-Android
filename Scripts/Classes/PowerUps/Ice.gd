...
const FIREBALL = preload("res://Instances/Prefabs/Projectiles/ice_ball.tscn")
...
func main(delta: float) -> void:
	if Input.is_action_just_pressed(CoopManager.get_player_input_str("run", player.player_id)) and not player.spinning and not player.riding_yoshi:
		throw_fire_ball(player.direction)
	if Input.is_action_just_pressed(CoopManager.get_player_input_str("spin_jump", player.player_id)) and player.is_on_floor():
		spin_fire_meter = 1
